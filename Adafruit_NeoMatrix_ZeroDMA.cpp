...
Adafruit_NeoMatrix_ZeroDMA::Adafruit_NeoMatrix_ZeroDMA(int w, int h,
  uint8_t pin, uint8_t matrixType, neoPixelType ledType) : Adafruit_GFX(w, h),
  Adafruit_NeoPixel_ZeroDMA(w * h, pin, ledType), type(matrixType),
...
Adafruit_NeoMatrix_ZeroDMA::Adafruit_NeoMatrix_ZeroDMA(uint8_t mW, uint8_t mH,
  ...
  neoPixelType ledType) : Adafruit_GFX(mW * tX, mH * tY),
...
void Adafruit_NeoMatrix_ZeroDMA::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_NeoMatrix_ZeroDMA::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
